...
tags:
  ...
  - analysis 
---
A conformal operatorname{tr}ansformation is one that preserves angles. In two dimensions, this is equivalent to being holomorphic and having a non-vanishing derivative.
There are tons of these operatorname{tr}ansformations (my personal favorites are Mobius transformations). In fact, the famous [Riemann mapping Theorem](https://en.wikipedia.org/wiki/Riemann_mapping_theorem)
...
In Euclidean space with the standard inner product, this matches the formula you know. Thus a linear operatorname{tr}ansformation $A: V \to V$
...
Thus a conformal linear operatorname{tr}ansformation is just an orientation and distance preserving map (called a special orthogonal matrix)
...
be a conformal linear operatorname{tr}ansformation!
...
How soperatorname{tr}ong is this restriction on the derivative? As it turns out, pretty strong.\
...
\begin{align} A^TB^TA+A^TBA &= \frac{2}{n}(\det (A))^{2/n -1} \det A \operatorname{tr} \left(A^{-1} \frac{dA}{dt}\right)\\
&=\frac{2}{n} (\det A)^{2/n} \operatorname{tr} (B) I.$$
...
We know $c_{iik}=c_{jjk}$ for all $i,j$ so set $c_k:=c_{iik}$ and $c = (c_1,\dots,c_n$). Using the symmeoperatorname{tr}ies again we get $c_{ikk} = -c_{kik} = -c_{kki}=-c_i$. 
